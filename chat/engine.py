...
class ChatQueryEngine:
    ...
    async def process_query_stream(
        ...
        try:
            ...
            logger.error("Error generating streaming chat answer: %s", exc, exc_info=True)
            err_msg = f"⚠️ The AI provider encountered a temporary error: {exc}. Please retry in a few moments."
            yield f"data: {json.dumps({'type': 'done', 'answer': err_msg, 'citations': [], 'session_id': session, 'grounded': False, 'latency_ms': round((time.perf_counter() - start_time) * 1000.0, 2)})}\n\n"
